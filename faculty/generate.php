...
if (!(empty($_POST["mobno"]) || empty($_POST["username"]) || empty($_POST["name"]))) {
    ...
    $user = md5($username);
    $sql1 = "SELECT * FROM `faculty` WHERE name = '$username'";
    ...
    if ($result1->num_rows > 1) {
        ...
        if ($row1['verified'] == 0) {
            $otp = getToken();
            if ((strtolower($row1['pname']) == strtolower($name)) && ($row1['mobile'] == $mobno)) {
                $dat = $row1['name'];
                $stamp = time();
                $sql2 = "UPDATE `faculty` SET nameh = '$user', otpverify = '$otp', stamp = '$stamp' WHERE name = '$username'";
        ...
        if ($pass === $cpass) {
            if (strlen($pass) > 5 || strlen($cpass) > 5) {
                ...
                if ($result1->num_rows == 1) {
                    ...
                    $ipaddr = getIPAddress();
                    if (strtolower($row1['otpverify']) == strtolower($otp)) {
                        $sql2 = "UPDATE `faculty` SET password = '$mpass', verified = 1, ipaddress = '$ipaddr' WHERE nameh = '$user'";
                        if (mysqli_query($link, $sql2)) {
...
if (!(empty($_POST['resend']))) {
    ...
    if ($result1->num_rows == 1) {
        ...
        $mobno = $row1['mobile'];
        $otp = getToken();
        $sql2 = "UPDATE `faculty` SET otpverify = '$otp' WHERE nameh = '$user'";
        if (mysqli_query($link, $sql2)) {
            $body = "Hey, " . $dat . "\nYour OTP is: " . $otp . "\nTo Register Into The Portal\n(Dont Share This With Anyone)\nWith Best Regards,\nOptimus/Medicus Patna.";
            $body = str_replace(" ", $space, $body);
            $body = str_replace("\n", $newline, $body);
...
<head>
    ...
    <title>Optimus Password Generate</title>
        ...
        <div class="container1">
            <?php if ($status == 0) { ?>
                <form action="" method="post">
                    <div class="formbox">
                        <div class="head"></div>
                        <h4 class="log">Faculty Password Generate</h4>
                        <div class="inputgrp">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <div class="input-group-text"><i class="fas fa-bars"></i></div>
                                </div>
                                <input type="text" name="username" class="form-control" minlength="3" placeholder="Registered username" required>
                            </div>
