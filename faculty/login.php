...
session_start();
if (isset($_SESSION['ssid']) && isset($_SESSION['name']) && isset($_SESSION['adm'])) {
    header("Location: panel.php?token=" . $_SESSION['ssid'] . "&page=Notice");
    exit;
}
$message = "";
$messtype = 0;
...
if (!(empty($_POST["name"]) || empty($_POST["pass"]))) {
    ...
    $user = md5($name);
    $sql1 = "SELECT * FROM `faculty` WHERE nameh = '$user'";
    $result1 = $link->query($sql1);
    if ($result1->num_rows > 1) {
        $message = "Multiple Same Value Exist";
        ...
        if ($row1['password'] == $mpass) {
            if ($row1['ban'] == 0) {
                ...
                if ($row1['sessionid'] == "0") {
                    $sid = getTokenOAuth();
                    $sql3 = "UPDATE `faculty` SET sessionid = '$sid', ipaddress = '$cip' WHERE nameh = '$user'";
                    if (mysqli_query($link, $sql3)) {
                        $_SESSION['ssid'] = $sid;
                        $_SESSION['name'] = $dat;
                        $_SESSION['adm'] = $dat2;
                        header("Location: panel.php?token=" . $sid . "&page=Notice"); //maybe
                        exit;
                    } else {
                    ...
                    $otp = getToken();
                    $otpexp = time() + 960;
                    $sql2 = "UPDATE `faculty` SET otp2fa = '$otp', exp2fa = '$otpexp' WHERE nameh = '$user'";
                ...
                $message = "You Account Have Been Blocked By Administrator!";
            }
        } else {
...
if (!(empty($_POST['key']))) {
    ...
    if ($result1->num_rows > 1) {
        ...
        $row1 = $result1->fetch_assoc();
        if (strtolower($row1['otp2fa']) == strtolower($key)) {
            $stamp = time();
            $sid = getTokenOAuth();
            $dat = $row1['name'];
            $dat2 = $row1['adm'];
            ...
            if ($row1['exp2fa'] > $stamp) {
                $sql2 = "UPDATE `faculty` SET otp2fa = 0, exp2fa = 0, sessionid = '$sid', ipaddress = '$cip' WHERE nameh = '$user'";
                if (mysqli_query($link, $sql2)) {
...
if (!(empty($_POST['resend']))) {
    ...
    if ($result1->num_rows == 1) {
        ...
        $otp = getToken();
        $exp = time() + 960;
        $sql2 = "UPDATE `faculty` SET otp2fa = '$otp', exp2fa = '$exp' WHERE nameh = '$user'";
        if (mysqli_query($link, $sql2)) {
            $body = "Hey, " . $dat . "\nYour OTP is: " . $otp . "\nTo Verify Your Login\nThis OTP is valid for 15min\n(Dont Share This With Anyone)\nWith Best Regards,\nOptimus/Medicus Patna";
            $body = str_replace(" ", $space, $body);
            $body = str_replace("\n", $newline, $body);
...
<head>
    ...
    <title>Optimus Faculty Login</title>
        ...
        <div class="container1">
            <?php if ($status == 0) { ?>
                <form action="" method="post">
                    <div class="formbox">
                        <div class="head"></div>
                        <h4 class="log">Faculty Login</h4>
                        <div class="inputgrp">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <div class="input-group-text"><i class="fas fa-bars"></i></div>
                                </div>
                                <input type="text" name="name" class="form-control" minlength="3" placeholder="Enter your username" required>
                            </div>
