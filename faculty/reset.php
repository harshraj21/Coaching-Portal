...
if (!(empty($_POST["name"]) || empty($_POST["mob"]))) {
    ...
    if ($result1->num_rows > 1) {
        ...
        if ($row1['verified'] == 1) {
            if ($row1['mobile'] == $mob) {
                ...
                $otp = getToken();
                $otpexp = time() + 960;
                $sql2 = "UPDATE `faculty` SET otpreset = '$otp', expreset = '$otpexp' WHERE nameh = '$user'";
        ...
        if ($pass === $cpass) {
            if (strlen($pass) > 5 || strlen($cpass) > 5) {
                ...
                if ($result1->num_rows > 1) {
                    ...
                    if(strtolower($row1['otpreset']) == strtolower($otp)){
                        $row1 = $result1->fetch_assoc();
                        $stamp1 = time();
                        if ($row1['expreset'] > $stamp1) {
                            ...
                            $sql2 = "UPDATE `faculty` SET password = '$mpass', otpreset = 0, expreset = 0, ipaddress = '$cip' WHERE nameh = '$user'";
                            if (mysqli_query($link, $sql2)) {
...
if (!(empty($_POST['resend']))) {
    ...
    if ($result1->num_rows == 1) {
        ...
        $otp = getToken();
        $exp = time() + 960;
        $sql2 = "UPDATE `faculty` SET otpreset = '$otp', expreset = '$exp' WHERE nameh = '$user'";
        if (mysqli_query($link, $sql2)) {
            $body = "Hey, " . $dat . "\nYour OTP is: " . $otp . "\nTo Reset The Password\nThis OTP is valid for 15min\n(Dont Share This With Anyone)\nWith Best Regards,\nOptimus/Medicus Patna";
            $body = str_replace(" ", $space, $body);
            $body = str_replace("\n", $newline, $body);
...
<head>
    ...
    <title>Optimus Recover Password</title>
                    ...
                    <div class="formbox">
                        ...
                        <div class="inputgrp">
                            <div class="input-group">
                                ...
                                <input type="text" name="name" class="form-control" minlength="3" placeholder="Registered username" required>
                            </div>
